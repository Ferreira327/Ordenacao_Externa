...
void quicksort(Data *V, int inicio, int fim)
{
    if (inicio < fim)
    {
        int p = inicio;
        ...
        {
            if ((strcmp(V[i].Area, pivot.Area) < 0) ||
                ((strcmp(V[i].Area, pivot.Area) == 0) && (V[i].geo_count < pivot.geo_count)))
            {
                swap(V[i], V[p]);
                p++;
            }
...
int nPessoasBin()
{
    ...
    int tamanhoArquivo = calcular.tellg();
    calcular.close();
    return tamanhoArquivo / sizeof(Data);
}
